...
namespace GreenshotPlugin.Core  {
	...
	public class WindowDetails : IEquatable<WindowDetails>{
		...
		public bool Visible {
			get {
				if (isApp) {
					return ModernUI.AppVisible(WindowRectangle);
				}
				if (isGutter) {
		...
		public ICapture CaptureDWMWindow(ICapture capture, WindowCaptureMode windowCaptureMode, bool autoMode) {
			...
			try {
				...
				if (!Maximised) {
					...
					User32.GetBorderSize(Handle, out borderSize);
		...
		public static WindowDetails GetActiveWindow() {
			IntPtr hWnd;
			// Use WinEventHook if we have this enabled (in development)
			if (GreenshotPlugin.Core.Capturing.WinEventHook.Instance.isHooked && GreenshotPlugin.Core.Capturing.WinEventHook.Instance.Windows.First != null) {
				hWnd = GreenshotPlugin.Core.Capturing.WinEventHook.Instance.Windows.First.Value.Handle;
			} else {
		...
		public static List<WindowDetails> GetMetroApps() {
			...
			foreach (IntPtr appHandle in ModernUI.MetroApps) {
				metroApps.Add(new WindowDetails(appHandle));
			}
			return metroApps;
		}
		...
		public static WindowDetails GetAppLauncher() {
			IntPtr appLauncher = ModernUI.AppLauncher;
