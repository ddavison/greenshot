...
namespace GreenshotPlugin.Core {
	...
	public static class ImageHelper {
				...
				for (int iIndex=0; iIndex<iCount; iIndex++) {
					...
					if (iWidth == 0 && iHeight == 0) {
						...
						using (MemoryStream destStream = new MemoryStream()) {
							...
							// The stream location is where a PNG is located
							using (Bitmap tmpBitmap = new Bitmap(destStream)) {
								// Clone to remove any references to the stream
								bmpPngExtracted = (Bitmap)Clone(tmpBitmap);
							}
						}
						break;
					}
		...
		public static Bitmap CloneArea(Image sourceImage, Rectangle sourceRect, PixelFormat targetFormat) {
			...
			// Clone property items, making sure if there is EXIF information this is not lost!
			try {
				if (sourceImage != null && sourceImage.PropertyItems != null) {
					foreach (PropertyItem propertyItem in sourceImage.PropertyItems) {
						newImage.SetPropertyItem(propertyItem);
					}
				}
			} catch (Exception clonePropertiesEx) {
				LOG.Warn("Problem cloning the PropertyItems to the image: ", clonePropertiesEx);
			}
			return newImage;
		}
