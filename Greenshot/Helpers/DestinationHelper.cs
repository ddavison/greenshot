	...
	public static class DestinationHelper {
		...
		private static List<IDestination> GetPluginDestinations() {
			...
			if (PluginHelper.Instance.HasPlugins) {
				foreach (var plugin in PluginHelper.Instance.Plugins) {
					if (plugin.IsValueCreated) {
						try {
							var dests = plugin.Value.Destinations();
							if (dests != null) {
								destinations.AddRange(dests);
							}
						} catch (Exception ex) {
							LOG.ErrorFormat("Couldn't get destinations from the plugin {0}", plugin.Metadata["name"]);
							LOG.Error(ex);
						}
					}
				}
