			...
			switch(captureMode) {
				...
				case CaptureMode.LastRegion:
					if (!conf.LastCapturedRegion.IsEmpty) {
						capture = WindowCapture.CaptureRectangle(capture, conf.LastCapturedRegion);
						// TODO: Reactive / check if the elements code is activated
						//if (windowDetailsThread != null) {
						//	windowDetailsThread.Join();
						//}
...
//				}
		...
		private void HandleCapture() {
			...
			if (destinationCount > 0) {
				...
				foreach(IDestination destination in captureDetails.CaptureDestinations) {
					if (PickerDestination.DESIGNATION.Equals(destination.Designation) || EditorDestination.DESIGNATION.Equals(destination.Designation)) {
						continue;
					}
				...
				if (captureDetails.HasDestination(Destinations.PickerDestination.DESIGNATION)) {
					ExportInformation exportInformation = DestinationHelper.ExportCapture(false, PickerDestination.DESIGNATION, surface, captureDetails);
					if (exportInformation.ExportMade && EditorDestination.DESIGNATION.Equals(exportInformation.DestinationDesignation)) {
						canDisposeSurface = false;
					}
				} else if (captureDetails.HasDestination(Destinations.EditorDestination.DESIGNATION)) {
					ExportInformation exportInformation = DestinationHelper.ExportCapture(false, EditorDestination.DESIGNATION, surface, captureDetails);
		...
		private bool CaptureActiveWindow() {
			...
			// Fix for Bug #3430560 
			conf.LastCapturedRegion = selectedCaptureWindow.WindowRectangle;
