...
namespace Greenshot {
	...
	public partial class ImageEditorForm : BaseForm, IImageEditor {
		...
		List<BidirectionalBinding> bindings = new List<BidirectionalBinding>();
		private void bindFieldControls() {
			bindings.Add(new BidirectionalBinding(btnFillColor, "SelectedColor", surface, FieldTypes.FILL_COLOR, NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(btnLineColor, "SelectedColor", surface, FieldTypes.LINE_COLOR, NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(lineThicknessUpDown, "Value", surface, FieldTypes.LINE_THICKNESS, DecimalIntConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(blurRadiusUpDown, "Value", surface, FieldTypes.BLUR_RADIUS, DecimalIntConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(magnificationFactorUpDown, "Value", surface, FieldTypes.MAGNIFICATION_FACTOR, DecimalIntConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(pixelSizeUpDown, "Value", surface, FieldTypes.PIXEL_SIZE, DecimalIntConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(brightnessUpDown, "Value", surface, FieldTypes.BRIGHTNESS, DecimalDoublePercentageConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(fontFamilyComboBox, "Text", surface, FieldTypes.FONT_FAMILY, NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(fontSizeUpDown, "Value", surface, FieldTypes.FONT_SIZE, DecimalFloatConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(fontBoldButton, "Checked", surface, FieldTypes.FONT_BOLD, NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(fontItalicButton, "Checked", surface, FieldTypes.FONT_ITALIC, NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(textHorizontalAlignmentButton, "SelectedTag", surface, FieldTypes.TEXT_HORIZONTAL_ALIGNMENT, HorizontalAlignmentConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(textVerticalAlignmentButton, "SelectedTag", surface, FieldTypes.TEXT_VERTICAL_ALIGNMENT, VerticalAlignmentConverter.GetInstance(), NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(shadowButton, "Checked", surface, FieldTypes.SHADOW, NotNullValidator.GetInstance()));
			bindings.Add(new BidirectionalBinding(obfuscateModeButton, "SelectedTag", surface, FieldTypes.PREPARED_FILTER_OBFUSCATE));
			bindings.Add(new BidirectionalBinding(highlightModeButton, "SelectedTag", surface, FieldTypes.PREPARED_FILTER_HIGHLIGHT));
		}
		...
		private void refreshFieldControls() {
			...
			if(surface.HasSelectedElements || surface.DrawingMode != DrawingModes.None) {
				// Update bindings
				foreach (BidirectionalBinding binding in bindings) {
					binding.Refresh();
				}
				btnConfirm.Visible = btnCancel.Visible = surface.IsElementWithFlagSelected(FieldFlag.CONFIRMABLE);
				...
				arrowHeadsLabel.Visible = arrowHeadsDropDownButton.Visible = surface.IsElementWithFieldTypeSelected(FieldTypes.ARROWHEADS);
			} else {
				hideToolstripItems();
			}
		...
		private void refreshEditorControls() {
			...
			if (surface.IsElementWithFieldTypeSelected(FieldTypes.FLAGS)) { // && ((FieldType.Flag)props.GetFieldValue(FieldType.FLAGS) & FieldType.Flag.CONFIRMABLE) == FieldType.Flag.CONFIRMABLE) {
		...
		void ArrowHeadsToolStripMenuItemClick(object sender, EventArgs e) {
			surface.ChangeFields(FieldTypes.ARROWHEADS, (ArrowHeadCombination)((ToolStripMenuItem)sender).Tag);
		}
